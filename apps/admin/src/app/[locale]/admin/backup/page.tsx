...
import { DownloadCloud, Database, HardDrive, CheckCircle2, AlertCircle } from 'lucide-react';
  ...
  const backups = [
    {
      ...
      description: 'Export all Firestore collections to a JSON archive',
      icon: Database,
    ...
    },
    {
      ...
      description: 'Backup all uploaded documents and PDFs',
      icon: HardDrive,
      lastBackup: 'No backups yet',
      status: 'pending' as const,
    },
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-gray-400">System data backup and restore</p>
      </div>
          ...
          return (
            <div key={b.id} className="rounded-xl border border-gray-800 bg-gray-900 p-6">
              <div className="flex items-start gap-4">
                <div className="rounded-lg bg-brand-500/20 p-3 text-brand-400">
                ...
                <div className="flex-1">
                  ...
                  <div className="mt-3 flex items-center gap-2">
                    <Badge variant={b.status === 'pending' ? 'pending' : 'active'}>
                      {b.status === 'pending' ? 'Not backed up' : 'Completed'}
                    </Badge>
                    {b.lastBackup !== 'No backups yet' && (
                      <span className="text-xs text-gray-600">{b.lastBackup}</span>
                    )}
                  </div>
      ...
      <div className="rounded-xl border border-gray-800 bg-gray-900 p-6">
        ...
        <p className="text-sm text-gray-500">
          Automated backups are configured via Firebase Console. Configure schedules and retention policies there.
        </p>
        <div className="mt-4 flex items-center gap-2 text-xs text-gray-600">
          <CheckCircle2 className="h-3.5 w-3.5 text-gray-500" />
          Daily backups are handled by Firebase platform
        </div>
      </div>
    </div>
  );
