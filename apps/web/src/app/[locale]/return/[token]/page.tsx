...
export default function ReturnActionPage() {
    ...
    async function process() {
      try {
        const res = await fetch('/api/workflow/action', {
          ...
          body: JSON.stringify({ token, action: 'return', comment: 'Returned via email' }),
        });
        const data = await res.json();
        if (res.ok && data.success) {
          setState('success');
          setMessage(`"${data.documentName || 'Document'}" has been returned.`);
        } else {
          setState('error');
          setMessage(data.error || 'Failed to process return');
        }
      } catch {
        setState('error');
      ...
      <div className="w-full max-w-md rounded-2xl bg-white p-8 shadow-lg text-center">
        {state === 'processing' && (
          <div className="space-y-4">
            ...
            <h2 className="text-lg font-semibold text-gray-900">Processing return...</h2>
          </div>
        )}
        {state === 'success' && (
          <div className="space-y-4">
            ...
            <h2 className="text-xl font-bold text-gray-900">Returned!</h2>
