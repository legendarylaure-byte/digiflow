  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <StatCard label="Total Users" value={stats.loading ? '...' : stats.totalUsers} icon={<Users className="h-5 w-5" />} color="violet" />
        <StatCard label="Active Documents" value={stats.loading ? '...' : stats.activeDocuments} icon={<FileText className="h-5 w-5" />} color="emerald" />
        <StatCard label="Pending Approvals" value={stats.loading ? '...' : stats.pendingApprovals} icon={<Clock className="h-5 w-5" />} color="amber" />
        <StatCard label="System Health" value={stats.error ? 'Error' : 'Online'} icon={<Activity className="h-5 w-5" />} color={stats.error ? 'rose' : 'emerald'} description="All services operational" />
      </div>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <div className="rounded-xl border border-gray-800 bg-gray-900 p-6">
          ...
          {stats.loading ? (
            <div className="space-y-3">{[1, 2, 3, 4].map((i) => <div key={i} className="h-8 animate-pulse rounded bg-gray-800" />)}</div>
          ) : (
                ...
                return (
                  <div key={item.label}>
                    <div className="flex items-center justify-between mb-1">
                      <div className="flex items-center gap-2"><div className={`h-2 w-2 rounded-full ${item.color}`} /><span className="text-sm text-gray-300">{item.label}</span></div>
        ...
        <div className="rounded-xl border border-gray-800 bg-gray-900 p-6">
          ...
          {stats.loading ? (
            <div className="space-y-3">{[1, 2, 3, 4, 5].map((i) => <div key={i} className="h-8 animate-pulse rounded bg-gray-800" />)}</div>
          ) : (
            <div className="space-y-3">
              {Object.entries(stats.usersByRole).sort(([, a], [, b]) => b - a).map(([role, count]) => {
      ...
      <div className="rounded-xl border border-gray-800 bg-gray-900 p-6">
        ...
        {stats.loading ? (
          <div className="space-y-3">{[1, 2, 3, 4, 5].map((i) => <div key={i} className="flex items-center justify-between"><div className="h-4 w-48 animate-pulse rounded bg-gray-800" /><div className="h-3 w-20 animate-pulse rounded bg-gray-800" /></div>)}</div>
            ...
            {stats.recentActivity.map((entry) => (
              <div key={entry.id} className="flex items-center justify-between rounded-lg border border-gray-800 px-4 py-3">
                ...
                <span className="text-xs text-gray-500">
                  {entry.timestamp instanceof Date ? entry.timestamp.toLocaleDateString('en-US', { month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' }) : ''}
                </span>
              </div>
            ))}
          </div>
